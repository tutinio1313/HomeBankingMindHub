var app = new Vue({
    ...
    methods:{
        getData: function(){
            Promise.all([axios.get("/api/loan"),axios.get("/api/client/current")])
            .then((response) => {
                //get loan types ifo
        ...
        apply: function(){
            axios.post("/api/loan", { loanId: this.loanTypeId, amount: this.amount, Payment: this.payments, NumberAccount: this.accountToNumber})
            .then(response => { 
                this.modal.hide();
                this.okmodal.show();
            })
